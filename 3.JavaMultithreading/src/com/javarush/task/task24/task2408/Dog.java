...
public class Dog implements Pet {
    ...
    public Sayable toSayable(final int i) {
        ...
        class DogPet extends SuperDog implements Sayable{
            private String getName = getSuperQuotes()+name+getSuperQuotes();
            @Override
            public String say() {
                StringBuilder doDog = new StringBuilder(getName);
                if (i<1){
                    doDog.append(" спит.");
                }
                else {
                    doDog.append(" лает г");
                    char[] buffA = new char[i];
                    Arrays.fill(buffA, 'а');
                    doDog.append(new String(buffA));
                    doDog.append("в! ");
                    doDog.append(formatter.format(new Date()));

                }
                return doDog.toString();
            }
        }
